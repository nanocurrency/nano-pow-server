...
if [[ "$OS" == 'Linux' ]]; then
	...
	aws s3 cp $TRAVIS_BUILD_DIR/build/nano_pow_server-*-win64.tar.gz s3://$AWS_BUCKET/pow-server/$TRAVIS_BRANCH/nano_pow_server-$TRAVIS_COMMIT-win64.tar.gz --grants read=uri=http://acs.amazonaws.com/groups/global/AllUsers
	aws s3 cp $TRAVIS_BUILD_DIR/build/nano_pow_server-*-win64.tar.gz s3://$AWS_BUCKET/pow-server/$TRAVIS_BRANCH/nano_pow_server-latest-win64.tar.gz --grants read=uri=http://acs.amazonaws.com/groups/global/AllUsers
fi
